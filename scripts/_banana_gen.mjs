// SCRATCH: chemical-word BANANA. Crescent silhouette, cylinder-style top-lit
// shading, deana tone-ramp + words popping black. coin bg.
...
const COLS = 168, ROWS = 96, HEIGHT_RATIO = 1.8;
const cxV = (COLS - 1) / 2;
// thick curved tube -> a real banana crescent (∩, gentle curve)
const CR = 150, TMAX = 24, AMAX = 0.58;
const Ccx = cxV, Ccy = ((ROWS - 1) / 2) * HEIGHT_RATIO - 16 + CR;

const shade = (x, y) => {
  const X = x, Y = y * HEIGHT_RATIO;
  const dx = X - Ccx, dy = Y - Ccy;
  const dist = Math.hypot(dx, dy);
  const ang = Math.atan2(dx, -dy);          // 0 = straight up from Cc
  if (Math.abs(ang) > AMAX) return null;
  const t = ang / AMAX;                      // -1..1 along the banana
  const taper = Math.sqrt(Math.max(0, 1 - t * t * t * t)); // flatter top, blunt tips
  const T = TMAX * (0.55 + 0.45 * taper);    // keep full body, blunt the ends
  const across = (dist - CR) / T;            // -1 inner/bottom .. +1 outer/top
  if (Math.abs(across) > 1) return null;
  // top-lit tube: outer/top bright, inner belly dark
  const lum = clamp((0.28 + 0.64 * (across + 1) / 2) * (0.82 + 0.18 * taper), 0, 1);
...
for (let y = 0; y < ROWS; y++) { const row = [];
  for (let x = 0; x < COLS; x++) {
    ...
    } else { const ch = word[wp++] ?? "·"; row.push({ ch, tone, isWord: true });
      if (wp >= word.length) { nextWord(); gap = 2 + Math.round(tone * 8); } }
  }
  grid.push(row);
}
...
writeFileSync(OUT, `<!doctype html><meta charset=utf-8><style>html,body{margin:0;background:#e8a317}.w{min-height:100vh;display:grid;place-items:center;padding:3vh 1vw}svg{width:min(96vw,1000px);height:auto}</style><div class=w>${svg}</div>`);
