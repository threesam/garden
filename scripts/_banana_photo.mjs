// SCRATCH: banana pixel-traced from the fetched photo. Yellow mask (largest
// blob + hole-fill, NO convex fill -> crescent preserved), luminance tone via
// deana ramp, chemical words pop in black. coin bg.
import sharp from "sharp";
import { writeFileSync } from "node:fs";
const SRC = "/Users/salvatoredangelo/.claude/jobs/69c81f6d/tmp/banana_url.webp";
...
// hole-fill: flood bg from borders; unreached non-mask cells = interior holes
...
writeFileSync(OUT,`<!doctype html><meta charset=utf-8><style>html,body{margin:0;background:#e8a317}.w{min-height:100vh;display:grid;place-items:center;padding:3vh 1vw}svg{width:min(97vw,1100px);height:auto}</style><div class=w>${svg}</div>`);
await sharp(Buffer.from(svg),{density:200}).png().toFile(PNG);
console.log("\n--- banana(photo) "+COLS+"x"+ROWS+" ---");
