...
const TRACKS = [
  ...
  { user: "sixtomidnightproductions", slug: "scraptheep", title: "Scrap the Eep" },
  { user: "threesam", slug: "the-hey-blinkins", title: "The Hey Blinkins" },
];

const CARD_HEIGHT = 380;

function trackEmbed(user: string, slug: string) {
  ...
  return (
    <div className="flex snap-x snap-mandatory overflow-x-auto scroll-pl-6 pb-3 pr-6 [scrollbar-color:var(--coin)_transparent] [scrollbar-width:thin] md:gap-9 md:overflow-x-visible md:pb-0 md:pl-9 md:pr-9">
      {TRACKS.map((t) => (
        <div
          key={`${t.user}/${t.slug}`}
          className="ml-6 flex-[0_0_69vw] snap-start overflow-hidden rounded-xl bg-zinc-800 md:ml-0 md:min-w-0 md:flex-1"
